...
Weak prediction: is a prediction which is not complex
...
![Boosting in Action (it wont take the incorrect predictions)](https://bradleyboehmke.github.io/HOML/10-gradient-boosting_files/figure-html/boosting-in-action-1.png)
...
1.  **Loss Function Optimized**\
    A loss function is used to measure the error between the model’s predictions and the true values. Gradient descent is applied to minimize this loss by adjusting model parameters. (is what the algorithm tries to optimize (Ex: to optimize r squeare, rmse)
...
The `mtry` parameter defines how many predictors are randomly chosen at each split. This adds diversity among trees (recommended to start with 30% to show how the model works and then we can increase the tunning) . Typical values range from 1 up to the total number of features. In XGBoost, this is sometimes referred to as `colsample_bytree`. (from 0 - 1)
...
-   Will easily overfit becuase it will take the noise from previous model so we need to be careful on training in.
...
xgb_spec <- boost_tree(
  ...
  learn_rate = tune()) %>% 
  set_engine("xgboost") %>%
  set_mode("regression") #to set model for classification or regression
#Specifying XgBoost as our model type, asking to tune the hyperparameters, tune will bring the best number of trees we need (hyperparameter)
...
We use Latin hypercube sampling to generate a diverse grid of hyperparameter combinations (random sampling technique for tunning hyperparameters):

```{r }
xgb_grid <- grid_latin_hypercube( #is less computational heavy than the one we used before (so will be faster), because we have 4 hyperparameters, the more hyperparameters the heavier computational power you need. It will optimize distribution and combinations in the levels specified. 
  tree_depth(),
  min_n(),
  learn_rate(),   
  trees(), #tell what type of hyperparameters
  size = 100 #telling the number of combinations I want of these hyperparameters
) 
...
ggplot(data = xgb_grid,
  ...
  geom_point(aes(color = factor(learn_rate),
             ...
             show.legend = FALSE) 
```
Four variables in this plot to asses where are the combinations.  
## 3. Model Tuning

```{r xgb_grid_result}
set.seed(76544)
#parallel processing
registerDoParallel(cores = parallel:: detectCores()-1) #use one less code so your computer dont crash

xgb_res <- tune_race_anova(object = xgb_spec, #tune race anova will take all combinations, run anova on it and will only choose the best parameters. 
                      preprocessor = weather_recipe,
                      resamples = resampling_foldcv,
                      grid = xgb_grid,
                      control = control_race(save_pred = TRUE)) 

stopImplicitCluster() #to stop parallel processing

beepr::beep()
xgb_res$.metrics[[2]]
```
We got rsq and rmse of all of them. Then we can choose what will be the best for us. 
